...
public class AIcontroller : MonoBehaviour
{
    ...
    private float radius = 6, brakPower;
    private readonly float DownForceValue = 100f;
    private float lastValue;
    private Rigidbody rigidbody;
    private readonly GameObject[] wheelMesh = new GameObject[4];
    ...
    private void calculateDistanceOfWaypoints()
    {
        ...
        {
            var difference = nodes[i].transform.position - position;
            var currentDistance = difference.magnitude;
    ...
    {
        var relative = transform.InverseTransformPoint(currentWaypoint.transform.position);
    ...
    {
        if (vertical != 0) rigidbody.linearDamping = 0.005f;
        if (vertical == 0) rigidbody.linearDamping = 0.1f;
        ...
        {
            engineRPM = Mathf.SmoothDamp(engineRPM, 1000 + Mathf.Abs(wheelsRPM) * 3.6f * gears[gearNum], ref velocity,
                smoothTime);
            test = false;
        }
    ...
    private void shifter()
    {
        ...
        if (engineRPM < minRPM && gearNum > 0) gearNum--;
    }
    ...
    private void moveVehicle()
    {
        ...
        else if (drive == driveType.rearWheelDrive)
        {
            ...
            for (var i = 0; i < wheels.Length; i++) wheels[i].brakeTorque = brakPower;
        }
        else
        {
    ...
    private void steerVehicle()
    {
        ...
        //steerAngle = Mathf.Rad2Deg * Mathf.Atan(2.55f / (turnRadius + (1.5f / 2))) * horizontalInput;
    ...
    {
        var wheelPosition = Vector3.zero;
        var wheelRotation = Quaternion.identity;
